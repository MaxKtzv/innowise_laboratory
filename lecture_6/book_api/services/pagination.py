...
def compute_offset(page: int, limit: int) -> int:
    """
    Calculate the offset for pagination based on the current page and
    the limit.

    This function is typically used to determine the starting point of
    records in a paged dataset by converting the current page number and
    the limit of items per page into an offset.
...
def clamp_limit(limit: int, max_limit: int) -> int:
    ...
    Param:
        limit (int): The value to be clamped.
        max_limit (int): The upper limit to clamp the value to.
    Return:
        int: The clamped value, which is the smaller of `limit` and
            `max_limit`.

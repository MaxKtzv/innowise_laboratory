...
from books1 import paginate_books
...
from schemas import BookCreate, BookRead, BookUpdate, PaginatedBook
from services import clamp_limit
...
def read_all_books(
    ...
    limit: Annotated[int, Query(ge=1, le=25, description="Items per page")] = 10,
) -> PaginatedBook:
    """
    ...
    """
    books = paginate_books(database, page, limit)
    if not books:
...
def search_book(
    ...
    total = database.exec(select(func.count()).select_from(query.subquery())).one()
...
def delete_book(database: db_dependency, book_id: Annotated[int, Path(ge=1)]) -> None:
    """
